...
############# Code comment start!!!
#############
############# Step 2: Load the Model1 model
#############
############# Step 2.1: load the Model1 model
ckp_path_Model1 <- "./Results/Willow-actions/cropped/checkpoints"
### a) Check the saved model weight
list.files(ckp_path_Model1)
### b) Then load the weights of best model, and re-evaluate:
model_Model1_best <- load_model_hdf5(
  file.path(ckp_path_Model1, "Model1_weights.19-0.66.hdf5")
)

############# Step 2.2: Re-evaluate the model on testing set using Model1 model
### Obatin the prediction probability
...
cat("The task for re-evaluating the model on testing set using Model1 model has been competed!\n")

#############
############# Step 3: Load the Model2 models
#############
############# Step 3.1: load the Model1 model
ckp_path_Model2 <- "./Results/Willow-actions/cropped/checkpoints"
### a) Check the saved model weight
list.files(ckp_path_Model2)
### b) Then load the weights of best model, and re-evaluate:
model_Model2_best <- load_model_hdf5(
  file.path(ckp_path_Model2, "Model2_weights.49-0.68.hdf5")
)

############# Step 3.2: Re-evaluate the model on testing set using Model1 model
### b) Obatin the prediction probability
result_pred_Model2 <- model_Model2_best %>% predict_generator(test_generator,
                                                              steps = ceiling(test_generator$samples/batch_size)) # 20*6/10=12
# result_pred_Model2
cat("The task for re-evaluating the model on testing set using Model2 model has been competed!\n")

#############
############# Step 4:Ensemble the Model1 and Model2 models
#############
### a) Obtain the final prediction results
final_result_pred <- 0.5*(result_pred_Model1 + result_pred_Model2)

### b) Obatin the prediction classes
result_pred_class_Model3 <- apply(final_result_pred, 1, function(x) {which.max(x)-1})  # 20*6/10=12
# result_pred_class_Model3
...
final_acc <- length(which(result_pred_class_Model3 == true_labels_test))/length(result_pred_class_Model3)
cat("The final acc is ",final_acc,"\n")
### c) Get the confusionmatrix
cfn_matrix_list <- confusionMatrix(factor(result_pred_class_Model3),factor(true_labels_test))
...
Results_data_filename <- './Results/Willow-actions/cropped/data/Model3_Results.Rdata'
save(final_result_pred,result_pred_class_Model3,cfn_matrix,cfn_matrix_prob,cfn_matrix_overall,cfn_matrix_classes,
...
# Save the ggplot
cfnMatrix_filename <- './Results/Willow-actions/cropped/figures/Model3_confusionMatrix_ggplot.png'
